...
def panel_B(ax):
    ...
    ax.plot(x, b0_m, "o-", color=LINE_MAIN, linewidth=1.0, markersize=3,
            markeredgewidth=0.3, label="Layer 0")
    ax.plot(x, b1_m, "s--", color=LINE_SEC, linewidth=1.0, markersize=3,
            markeredgewidth=0.3, label="Layer 1")

    for si, sz in enumerate(sizes):
        for v in b0[sz]:
            ax.scatter(x[si] + np.random.uniform(-jitter, jitter), v,
                       s=2, marker=".", color=LINE_MAIN, alpha=SCATTER_ALPHA,
