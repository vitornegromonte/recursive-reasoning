"""Figure: Tufte Data-Ink Maximization style fork.
...
Palette: pure black (#000000), mid-grey (#666666), dark grey (#333333)
"""
...
def panel_A(ax):
    ...
    ax.errorbar(x, late_m, yerr=late_s, fmt="o-", color=LINE_MAIN,
                capsize=1, capthick=0.3, linewidth=1.0, markersize=3,
                markeredgewidth=0.3, label="Late stability")
    ax.errorbar(x, cka_m, yerr=cka_s, fmt="o--", color=LINE_SEC,
                capsize=1, capthick=0.3, linewidth=1.0, markersize=3,
                markeredgewidth=0.3, label="CKA(t\u2080, t\u2099)")
...
def panel_B(ax):
    ...
    ax.errorbar(x, dd_b0_m, yerr=dd_b0_s, fmt="o-", color=LINE_MAIN,
                linewidth=1.0, markersize=3, markeredgewidth=0.3,
                capsize=0, elinewidth=0.5, label="Layer 0")
    ax.errorbar(x, dd_b1_m, yerr=dd_b1_s, fmt="s-", color=LINE_SEC,
                linewidth=1.0, markersize=3, markeredgewidth=0.3,
                capsize=0, elinewidth=0.5, label="Layer 1")
    ...
    for si, sz in enumerate(sizes):
        if sz not in lin_b0 or not lin_b0[sz]:
            continue
        for v in lin_b0[sz]:
            ax.plot(x[si] + np.random.uniform(-jitter, jitter), v, ".",
                    color=LINE_MAIN, markersize=3, alpha=0.35,
                    markeredgewidth=0)
        for v in lin_b1[sz]:
            ax.plot(x[si] + np.random.uniform(-jitter, jitter), v, ".",
                    color=LINE_SEC, markersize=3, alpha=0.35,
                    markeredgewidth=0)
...
def panel_C(ax):
    ...
    ax.errorbar(x, means, yerr=stds, fmt="none",
                ecolor=LINE_MAIN, elinewidth=0.6, capsize=0)

    # White horizontal tick lines through bars at major intervals
    for tick_val in [0.05, 0.10, 0.15, 0.20]:
        ax.axhline(y=tick_val, color=BG_COLOR, linewidth=1.0)
...
def main():
    plt.rcParams.update({
        ...
        "font.sans-serif": ["Gill Sans", "Helvetica", "Palatino"],
        "font.size": 10,
        "axes.titlesize": 12,
        "axes.labelsize": 11,
    ...
    fig.savefig(OUT / "Figure_tufte.pdf")
    fig.savefig(OUT / "Figure_tufte.png")
    plt.close(fig)
    print(f"Saved Figure_tufte.pdf / .png  ({OUT})")
