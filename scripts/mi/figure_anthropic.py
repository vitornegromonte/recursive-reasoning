"""Figure: Anthropic Academic Editorial style fork.
...
Palette: warm cream paper (#FBF9F6), ink charcoal (#191816), burnt rust (#E0533C)
"""
...
TITLE_FONT = ["Georgia", "Times New Roman", "Palatino"]
...
def panel_B(ax):
    ...
    for si, sz in enumerate(sizes):
        for v in b0[sz]:
            ax.scatter(x[si] + np.random.uniform(-jitter, jitter), v,
                       s=8, color=LINE_MAIN, alpha=SCATTER_ALPHA,
    ...
    ax.legend(loc="lower left", frameon=False)
    ax.axhline(y=0, color=AXIS_TEXT, linewidth=0.5)
    ax.set_ylim(-1.0, 1.0)
    ax.set_xticks(x)
    ax.set_xticklabels([SIZE_LABELS[s] for s in sizes])
...
def main():
    ...
    # No suptitle per Anthropic style
    ...
    fig.savefig(OUT / "Figure_anthropic.pdf")
    fig.savefig(OUT / "Figure_anthropic.png")
    plt.close(fig)
    print(f"Saved Figure_anthropic.pdf / .png  ({OUT})")
