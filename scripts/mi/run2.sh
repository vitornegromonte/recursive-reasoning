...
run_experiments() {
    ...
    for entry in "${EXPERIMENTS[@]}"; do
        ...
        # Exp9 (attention circuit) is specific to ARC, skip for Sudoku
        if [[ "$script" == "attention_circuit.py" && "$domain" != "arc" ]]; then
            continue
        fi
...
echo ""
echo ">>> exp11: mixer visualisation..."
for domain_dir in "${OUT_SUDOKU}" "${OUT_ARC}"; do
    exp10_dir="${domain_dir}/exp10"
    exp11_dir="${domain_dir}/exp11"
    domain_label="$(basename "$domain_dir")"
    if [[ -d "$exp10_dir" ]]; then
        echo "  Running mixer_viz for ${domain_label} ..."
        python3 scripts/mi/mixer_viz.py \
            --results-dir "$exp10_dir" \
            --output-dir "$exp11_dir" \
            --domain "$domain_label"
    else
        echo "  [skip] ${exp10_dir} not found"
    fi
done
